...
厳守: 入力されたタイトルの件数と、出力するJSONオブジェクトの件数は、必ず同一にしてください。（入力が${batchSize}件なら、必ず${batchSize}件のJSONオブジェクトを返します）
[
  {
      ...
      messages: [
        {
          // ★★★ プロンプト修正点 (構文エラー修正) ★★★
          role: "system",
          content: "あなたは、DiscogsやMusicBrainzの知識を持つ、音楽カタログ解析AIです。あなたの最優先タスクは、絶対に空白（空欄）や安易なUnknownを返さないことです。提供されたリストを分析し、各項目から「アーティスト名」「リリースタイトル」「原産国」をデータベース検索のように正確に特定します。特にリリースタイトルが空白になりそうな場合は、代わりに元のタイトルを返すよう厳密に処理してください。"
        },
